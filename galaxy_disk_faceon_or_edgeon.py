...
# -*- coding: utf-8 -*-
"""
galaxy_disk_faceon_or_edgeon.py
Tintin Nguyen
Final Project
This script classifies disk galaxies into 2 groups: edge-on or face-on
"""
import numpy as np
import pylab as plt
...
while i < np.shape(features)[0]:
    ...
    if gtype == 4 or gtype == 5 or gtype == 6:
        features[i, 22] = 0 # edge on
        i += 1
    elif gtype == 1 or gtype == 7 or gtype == 8 or gtype == 9:
        features[i, 22] = 1 # face on
        i += 1
    else:
...
ax.xaxis.set_ticklabels(['Edge-on', 'Face-on'])
ax.yaxis.set_ticklabels(['Edge-on', 'Face-on'])
...
fig, ax = plt.subplots(figsize=(15,10))
ax.hist(var0_a, bins=20, range = (0.045, 0.075), histtype='step', color='r', density = True, label = "Edge-on", linewidth=2)
ax.hist(var0_b, bins=20, range = (0.045, 0.075),histtype='step', color='b', density = True, label = "Face-on", linewidth=2)
ax.set(title='Form factor', ylabel='density')
plt.legend(loc = "upper left")

fig, ax = plt.subplots(figsize=(15,10))
ax.hist(var1_a, bins=40, range=(0,0.6), histtype='step', color='r', density = True, label = "Edge-on", linewidth=2)
ax.hist(var1_b, bins=40, range=(0,0.6), histtype='step', color='b', density = True, label = "Face-on", linewidth=2)
